...
def evaluate(model, loader, mode, cfg, ddp=False):
    ...
    with torch.no_grad():
        for img_np, img, mask, id in tqdm(loader, desc='Inference'):
            ...
            if mode == "sliding_window":
                ...
                count = torch.zeros(b, 1, h, w).cuda()  # 计数矩阵
                size = cfg["crop_size"]
                step = int(size * 2 / 3)
                ...
                while row * step < h:
                    col = 0
                    while col * step < w:
                        h_start = min(row * step, h - size)
                        w_start = min(col * step, w - size)
                        h_end = min(h_start + size, h)
                        w_end = min(w_start + size, w)
                        
                        sub_input = img[:, :, h_start:h_end, w_start:w_end]
                        pred_logits = model(sub_input)
                        
                        final[:, :, h_start:h_end, w_start:w_end] += pred_logits
                        count[:, :, h_start:h_end, w_start:w_end] += 1
                        
                        col += 1
                    row += 1
                
                pred = (final / count.clamp(min=1)).argmax(dim=1)
