...
class LoRA_ViT_timm(nn.Module):
    def __init__(self, vit_model: timm_ViT, r: int, alpha: int, num_classes: int = -1, lora_layer=None):
...
if __name__ == "__main__":  # Debug
    ...
    parser.add_argument('--use_ckpt', default=False)
    ...
    lora_model = LoRA_ViT_timm(model, r=4, alpha=4, num_classes=-1)

    num_params = sum(p.numel() for p in lora_model.parameters() if p.requires_grad)
    print(f"trainable parameters: {num_params}") #trainable parameters: 86859496

    output = lora_model(img)
    print(len(output))
