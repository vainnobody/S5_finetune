...
class DinoVisionTransformer(nn.Module):
    ...
    #         parsed = urlparse(self.pretrained)
    #         if parsed.scheme in ("https", "http", "file"):
    #             state_dict = torch.hub.load_state_dict_from_url(
    #                 self.pretrained, map_location="cpu", check_hash=self.check_hash
    #             )
    #         else:
    ...
    #             print_log(f"Loaded DINOv3 pretrained weights for {self.arch}: {msg}", logger='current')

    #     else:
    #         hash_suffix = self.cfg["hashes"].get(self.weight_key)
    #         url = _make_pretrained_url(arch=self.arch, weights=self.weight_key, hash_suffix=hash_suffix)
    #         state_dict = torch.hub.load_state_dict_from_url(url, map_location="cpu", check_hash=self.check_hash)
    ...
    def forward(
        ...
        feats = self.forward_features(x, masks=masks)
