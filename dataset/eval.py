...
class SemiDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'val':
            ...
            return img, mask, np_img, id
            # return img, mask
        
        img, mask = resize(img, mask, (0.5, 2.0))
        # ignore_value = self.ignore_value if self.mode == 'train_l' else 254
        ...
        # return self.to_tensor(np.uint8(img_w)), self.to_tensor(np.uint8(img_s1)), ignore_mask
        return np.array(img_w), normalize(img_w), img_s1, ignore_mask, mask
